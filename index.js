const getBrightness = () => {
  let transitionHour = 20 // Time in the evening when the lights begin to dim.
  let sunriseHour = 6 // Time in the morning when the lights return to peak brightness.

  const maxSettings = {
    // % Brightness levels at peak brightness.
    medium: 100,
    dim: 100,
    veryDim: 100,
  }
  ...
  const minSettings = {
    // % Brightness levels at dim brightness. Can assign rooms to profiles, e.g. bedroom to veryDim.
    medium: 60,
    dim: 40,
    veryDim: 1,
  }
  ...
  if (currentHours === transitionHour) {
    ...
    return {
      medium:
        maxSettings['medium'] -
        Math.round((100 - minSettings['medium']) * (currentMinutes / 60)),
      dim:
        maxSettings['dim'] -
        Math.round((100 - minSettings['dim']) * (currentMinutes / 60)),
      veryDim:
        maxSettings['veryDim'] -
        Math.round((100 - minSettings['veryDim']) * (currentMinutes / 60)),
    }
  }
...
const handleSchedule = async () => {
  ...
  console.log(
    `Set brightness levels to medium: ${brightness['medium']}%, dim: ${brightness['dim']}%, veryDim: ${brightness['veryDim']}%`,
  )
